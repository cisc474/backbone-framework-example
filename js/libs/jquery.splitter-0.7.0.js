			...
			position : (function() {
				if (settings.orientation == 'vertical') {
					return function(n, silent) {
						...
						if (n === undefined) {
							n = panel_1.outerWidth() + sw;
						}
						sw /= 2;
						position = get_position(n);
						splitter.css('left', position - sw);
						panel_1.outerWidth(position - sw);
						var nTotal = self.outerWidth();
						if(nTotal == 0)
						{
							nTotal = jQuery("body").innerWidth();
						}
						panel_2.outerWidth(nTotal - position - sw);

						if (!silent) {
							self.find('.splitter_panel').trigger(
									'splitter.resize');
						}
						return self;
					};
							...
							function(e) {
								if (current_splitter !== null) {
									...
									if (current_splitter.orientation == 'vertical') {
										...
										} else if (x >= current_splitter
												.width()
												- limit) {
											x = current_splitter.width()
													- limit - 1;
										}
										if (x > current_splitter.limit
												&& x < current_splitter.width()
											...
											current_splitter.find(
													'.splitter_panel').trigger(
													'splitter.resize');
											return false;
										}
										...
										} else if (y >= current_splitter
												.height()
												- limit) {
											y = current_splitter.height()
													- limit - 1;
										}
										if (y > current_splitter.limit
												&& y < current_splitter
